        ...
        if (e instanceof IntExp) {
            ...
            if (def == null) {
                ...
                if (def.parameters.length != asFunc.parameters.length) {
                    ...
                    for (int index = 0; index < def.parameters.length; index++) {
            ...
            if (typeofExp(asCast.exp) instanceof VoidType) {
                throw new TypeErrorException("Cannot cast void: " + asCase.exp);
            } else {
                return e.type;
            }
            ...
            if (expType instanceof StructureType) {
                ...
                if (dec == null) {
                    throw new TypeErrorException("No such structure with name: " + dec.name);
                } else {
                    ...
                    throw new TypeErrorException("No such field defined: " + fieldBeingAccessed.name);
                }
            } else {
                throw new TypeErrorException("Expected structure; got: " + expType.toString());
            }
        }
    } // typeofExp
} // Typechecker
